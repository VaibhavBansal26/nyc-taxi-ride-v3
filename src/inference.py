...
def get_model_predictions(model, features: pd.DataFrame) -> pd.DataFrame:
    ...
    # ensure lag columns for weekly-average transformer
    ...
    if "pickup_hour" not in features.columns:
        raise ValueError(
            ...
            f"Columns: {list(features.columns)[:30]}"
        )
...
def load_batch_of_features_from_store(current_date: datetime) -> pd.DataFrame:
    ...
    if ts_data.empty:
        raise ValueError(
            "load_batch_of_features_from_store: no rows in requested window. "
            "Check feature group dates/timezones or widen the fetch window."
        )
...
def fetch_hourly_rides(hours):
    ...
    fs = get_feature_store()
    fg = fs.get_feature_group(name=config.FEATURE_GROUP_NAME, version=1)
    q = fg.select_all().filter(fg.pickup_hour >= current_hour)
    return q.read()
