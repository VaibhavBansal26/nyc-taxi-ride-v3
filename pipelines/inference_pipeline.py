...
def _read_timeseries_from_store(fs, fetch_data_from, fetch_data_to):
    """
    Try Feature View; if FV missing/None/empty, fall back to Feature Group.
    Return DataFrame with ['pickup_location_id','pickup_hour','rides'] clipped to window.
    """
    # 1) Try Feature View
    ts = pd.DataFrame()
    try:
    ...
    if ts.empty:
        ...
        if "pickup_hour" not in df.columns:
            raise ValueError(
                "Feature Group does not contain 'pickup_hour'. "
    ...
    if not ts.empty:
        print(
            f"[inference] Retrieved {len(ts)} rows. "
            f"pickup_hour range: {ts['pickup_hour'].min()} .. {ts['pickup_hour'].max()}"
        )
    return ts
...
if ts_data.empty:
    raise ValueError(
        "inference_pipeline: no rows from feature view or feature group in requested window. "
        "Check FV config or FG contents/timezones."
    )

ts_data = ts_data.sort_values(["pickup_location_id", "pickup_hour"]).reset_index(drop=True)

# Build sliding-window features (672 hours for 4 weekly lags)
features = transform_ts_data_info_features(
    ts_data,
    feature_col="rides",
    window_size=24 * 28,  # 672
