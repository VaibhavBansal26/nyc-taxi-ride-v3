...
# Try FV (create if missing). If any error, fallback to FG read.
ts_data = None
try:
    fv = ensure_feature_view(fs)  # may create if missing
    ts_data = fv.get_batch_data(
        start_time=(fetch_data_from - timedelta(days=1)),
        end_time=(fetch_data_to + timedelta(days=1)),
    )
...
if ts_data is None or ts_data.empty or "pickup_hour" not in ts_data.columns:
    # FG fallback
    fg = fs.get_feature_group(name=config.FEATURE_GROUP_NAME, version=config.FEATURE_GROUP_VERSION)
    df = fg.select_all().read()
    if "pickup_hour" not in df.columns:
        raise ValueError("FG read ok but 'pickup_hour' column missing.")
    df = df.copy()
    df["pickup_hour"] = pd.to_datetime(df["pickup_hour"], errors="coerce", utc=True)
    ts_data = df[df["pickup_hour"].between(fetch_data_from, fetch_data_to)]
    print(
        f"[inference_pipeline] FG fallback returned {len(ts_data)} rows "
        f"({ts_data['pickup_hour'].min() if not ts_data.empty else None} .. "
        f"{ts_data['pickup_hour'].max() if not ts_data.empty else None})"
    )
else:
    ts_data = ts_data.copy()
    ts_data["pickup_hour"] = pd.to_datetime(ts_data["pickup_hour"], errors="coerce", utc=True)
...
fg_pred = get_feature_store().get_or_create_feature_group(
    name=config.FEATURE_GROUP_MODEL_PREDICTION,
    version=1,
    description="Predictions from LGBM Model",
    primary_key=["pickup_location_id", "pickup_hour"],
...
fg_pred.insert(preds, write_options={"wait_for_job": False})
print("[inference_pipeline] Prediction insert submitted.")
